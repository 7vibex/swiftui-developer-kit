...
Use the skills under `.agents/skills/` as reusable workflows for SwiftUI, iOS, iPadOS, macOS, SwiftData, canvas/PencilKit, diagnostics, localization, app security/privacy, Liquid Glass, accessibility, testing, release, and PR work.
...
Use the swiftui-diagnostics-builder skill. Add privacy-safe diagnostics with logs, breadcrumbs, app-state snapshots, and issue-report export.
Use the swiftui-localization-auditor skill. Audit every supported language for runtime and formatting gaps.
Use the apple-app-security-privacy-auditor skill. Audit app-side data flows, credentials, networking, permissions, and disclosures.
