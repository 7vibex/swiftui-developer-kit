...
- Use `../../docs/capture-consent-protocol.md` before any capture.
...
- See the fictional API-verification example at `../../examples/liquid-glass-api-verification.md`.
...
8. Build or run only when the user or host project has approved that workflow; otherwise report the static verification completed and the approval still needed.
9. When build/run is approved, use the host project's required simulator or device and record the result.
...
- OS support decision: ask before implementation whether the oldest supported OS keeps its existing UI, gets a non-glass fallback, or is intentionally dropped by raising the minimum OS.
