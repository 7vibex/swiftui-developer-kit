...
- Worked fictional example: `../../../examples/canvas-engine-auditor-example.md`
...
11. When code changes are made in an Apple app project, build and run only if the user or host repository has approved it; then use the required iPad simulator according to repository instructions. Otherwise record the static verification completed and the approval still needed. Ask before screenshots or Computer Use.
