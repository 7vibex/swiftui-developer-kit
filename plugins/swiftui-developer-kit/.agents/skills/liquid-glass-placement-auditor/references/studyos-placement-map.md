...
```text
Use the swiftui-feature-builder skill. Convert only the Canvas floating tool palette to Liquid Glass, keep the drawing canvas opaque, preserve the oldest-supported-OS UI path, add a Reduce Transparency fallback, then build and run only when the user or host project has approved it.
```
